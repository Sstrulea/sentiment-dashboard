...
def test_cad_gdp_qoq_has_the_monthly_override(indicators_cfg):
    ...
    assert cfg.get("frequency_overrides") == {"CAD": "monthly"}
...
def test_no_other_currency_gdp_qoq_frequency_or_max_age_changes(indicators_cfg):
    """The override is CAD-only. Every other currency applicable to
    gdp_qoq must resolve to the same frequency/max_age as the board
    default (quarterly/110d) -- confirmed unaffected."""
    cfg = indicators_cfg["indicators"]["gdp_qoq"]
    defaults = indicators_cfg["defaults"]
    for ccy in CCYS:
        if ccy == "CAD":

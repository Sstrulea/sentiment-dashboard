...
def a3_windows(src: str, bank: str, x: dict) -> list[dict]:
    ...
    if src == "atlantafed_mpt":
        ...
        for c in x["contracts"]:
            w.append({"start": c["expiry"], "end": c["expiry"] + timedelta(days=91), "rate": c["implied"]})
...
def report_a3_implied(probes: list[Probe], today: date) -> None:
    ...
    for bank in banks:
        ...
        for src in ("atlantafed_mpt", "mx_corra_expectations", "jpx_settlement_csv", "asx_markit_json"):
            ...
            for lbl, mm in (("last-2026", m26), ("last-2027", m27)):
                ...
                win = next((r for r in w if r["start"] >= eff - timedelta(days=WINDOW_TOL_DAYS)), None)
                if win is None:
    ...
    if us_m and mpt and tr:
        ...
        win = next((r for r in a3_windows("atlantafed_mpt", "USD", mpt.extra) if r["start"] >= eff), None)
        cv = tr.extra["curve"]
    ...
    if cad:
        ...
        win = next((r for r in a3_windows("mx_corra_expectations", "CAD", cad.extra) if r["start"] >= cm.decision), None)
...
def main(argv: list[str] | None = None) -> int:
    ...
    for sec in args.section or list(SECTIONS):
        ...
        if sec == "a3":
            ...
            _print_negative(ps)
    if len(args.section or SECTIONS) == len(SECTIONS):
